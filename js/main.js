...
function partBonus(p){
 const m=MUTATIONS.find(x=>x.id===p.id);
 if(!m)return {dps:0,tap:0};
 const tier=Math.max(1,p.tier||1);
 return {dps:m.dps*tier,tap:m.tap*tier};
}
function totals(){
 let dps=0,tap=1;
...
$("doubleBtn").addEventListener("click",()=>showRewardedAd(()=>{state.boostUntil=Date.now()+15*60*1000;state.boostType="double";saveState(state);render();toast("DNA production doubled for 15 minutes!")}));
